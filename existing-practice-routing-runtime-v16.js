...
function aggregateExistingPractice(errors) {
  ...
  for (const error of errors) {
    ...
    if (!rec) continue;
    const row = families.get(rec.family) || {
      family: rec.family,
      familyData: rec.familyData,
      matches: 0,
      routes: new Map()
    };
    row.matches += 1;
    const routeKey = `${rec.primary.id}|${rec.transfer.id}`;
    const route = row.routes.get(routeKey) || { rule: rec, count: 0 };
    route.count += 1;
    row.routes.set(routeKey, route);
...
function renderErrorNotebookRouting(core) {
  ...
  document.querySelectorAll('#main .error-item').forEach(item => {
    ...
    const marker = `${rec.primary.id}|${rec.transfer.id}`;
    ...
    route.innerHTML = `<span class="small muted">Already taught · review before the next test retry</span><button class="btn soft small-btn" data-lesson="${esc(rec.primary.id)}">Review ${esc(rec.primary.id)}</button><button class="btn ghost small-btn" data-lesson="${esc(rec.transfer.id)}">Practise ${esc(rec.transfer.id)}</button>`;
