...
function allRepairLessons() {
  return [...REPAIR_LESSONS, ...V16_SKILL_REPAIR_LESSONS];
}
...
function skillLabel(skill) {
  return ({ vocabulary:'Vocabulary', grammar:'Grammar', reading:'Reading', listening:'Listening' })[skill] || skill;
}
function questionHTML(lesson, question, index, saved = {}) {
  const correct = saved.checked && saved.selected === question.answer;
  const context = question.context ? `<div class="reading-passage" style="margin-bottom:14px"><p>${esc(question.context)}</p></div>` : '';
  return `<div class="quiz-card">${context}<div class="q-title">${esc(question.prompt)}</div><div class="options">${question.options.map((option,i)=>`<button class="option ${saved.checked ? (option===question.answer?'correct':(option===saved.selected?'wrong':'')) : (option===saved.selected?'selected':'')}" data-lrv="repair-option" data-rid="${lesson.id}" data-q="${index}" data-value="${esc(option)}" ${saved.checked?'disabled':''}><span class="option-letter">${String.fromCharCode(65+i)}</span><span>${esc(option)}</span></button>`).join('')}</div><div class="cluster"><button class="btn small-btn ${saved.checked?'soft':'primary'}" data-lrv="repair-check" data-rid="${lesson.id}" data-q="${index}" ${!saved.selected||saved.checked?'disabled':''}>${saved.checked?'Checked':'Check'}</button>${saved.checked&&!correct?`<button class="btn primary small-btn" data-lrv="repair-retry" data-rid="${lesson.id}" data-q="${index}">Retry</button>`:''}</div>${saved.checked?`<div class="feedback ${correct?'correct':'wrong'}"><strong>${correct?'Correct':'Not yet'}</strong><br>${esc(question.rationale)}</div>`:''}</div>`;
}

function evidenceHTML(lesson) {
  if (!lesson.evidence) return '';
  return `<div class="callout"><strong>Why this repair exists</strong><br>${lesson.evidence.auditedQuestions} tagged ${esc(lesson.skill)} questions in the ${esc(lesson.evidence.auditDate)} bank audit belong to the ${esc(lesson.evidence.family)} family.</div>`;
}

function mediaHTML(lesson) {
  if (!lesson.media?.length) return '';
  return `<section class="lesson-section"><h2>2. Listen in Practice Mode</h2><p class="muted">These are existing production recordings from the Question Type Labs. Replay is allowed here because this is Repair, not Test Mode.</p><div class="stack">${lesson.media.map(item=>`<article class="card subtle"><div class="eyebrow">${esc(item.label)}</div><p>${esc(item.instruction || '')}</p><audio controls preload="metadata" src="${esc(item.src)}" style="width:100%"></audio>${item.transcript?`<details style="margin-top:10px"><summary>Transcript — use after your first attempt</summary><p class="small muted" style="margin-top:8px">${esc(item.transcript)}</p></details>`:''}</article>`).join('')}</div></section>`;
}

function renderRepairRoute() {
  const match = location.hash.match(/^#\/lesson\/((?:VG|RR|LR)\d+)$/);
  if (!match) return;
  const main = document.querySelector('#main');
  if (!main) return;
  const lesson = allRepairLessons().find(item => item.id === match[1]);
  if (!lesson) return;
