...
const ALL_LABS = [...QUESTION_TYPE_LABS, ...QUESTION_TYPE_LABS_V2];
...
function removeLabsFromCoreLearn() {
  ...
  const titles = new Set(ALL_LABS.map(l => l.title));
...
function labCard(l, completed) {
  return `<article class="card lesson-card clickable">
...
function injectLab() {
  ...
  const done = ALL_LABS.filter(l => completed.has(l.id)).length;
  const reading = ALL_LABS.filter(l=>l.skill==='reading').map(l=>labCard(l,completed)).join('');
  const listening = ALL_LABS.filter(l=>l.skill==='listening').map(l=>labCard(l,completed)).join('');
  const section = document.createElement('section');
  section.dataset.questionTypeLabIndex = 'true';
  section.style.marginTop = '24px';
  section.innerHTML = `<div class="page-head" style="margin-bottom:14px"><div><div class="eyebrow">Question Type Lab · 12 units</div><h2>Train one exam decision at a time.</h2><p class="muted">These labs are separate from the 30-unit core curriculum. Checked answers still contribute Reading/Listening evidence and errors enter the same Repair / Review loop.</p></div><div><span class="chip primary">${done}/${ALL_LABS.length} complete</span></div></div>
  <div class="eyebrow" style="margin:18px 0 10px">Reading · 6 labs</div><div class="grid two">${reading}</div>
  <div class="eyebrow" style="margin:24px 0 10px">Listening · 6 labs</div><div class="grid two">${listening}</div>`;
...
function apply(){ removeLabsFromCoreLearn(); injectLab(); }
