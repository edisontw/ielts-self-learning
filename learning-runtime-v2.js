...
function saveAdaptive(state) { localStorage.setItem(ADAPTIVE_KEY, JSON.stringify(state)); }
...
function baseWeakness(core, skill) {
  ...
  if (['reading','listening','vocabulary','grammar'].includes(skill)) {
    return p[skill] == null ? 0.45 : (6 - p[skill]) / 6;
  }
  if (skill === 'writing') {
...
function candidates() {
  return [...CORE_LESSON_META, ...REPAIR_LESSONS.map(x => ({...x, targetRelevance: x.skill === 'grammar' ? 0.8 : 0.75}))];
}
...
function repairQuestion(lesson, q, index, saved = {}) {
  return `<div class="quiz-card"><div class="q-title">${esc(q.prompt)}</div><div class="options">${q.options.map((o,i)=>`<button class="option ${saved.checked ? (o===q.answer?'correct':(o===saved.selected?'wrong':'')) : (o===saved.selected?'selected':'')}" data-lrv="repair-option" data-rid="${lesson.id}" data-q="${index}" data-value="${esc(o)}" ${saved.checked?'disabled':''}><span class="option-letter">${String.fromCharCode(65+i)}</span><span>${esc(o)}</span></button>`).join('')}</div><button class="btn small-btn ${saved.checked?'soft':'primary'}" data-lrv="repair-check" data-rid="${lesson.id}" data-q="${index}" ${!saved.selected||saved.checked?'disabled':''}>${saved.checked?'Checked':'Check'}</button>${saved.checked ? `<div class="feedback ${saved.selected===q.answer?'correct':'wrong'}"><strong>${saved.selected===q.answer?'Correct':'Not yet'}</strong><br>${esc(q.rationale)}</div>` : ''}</div>`;
}
function renderRepairRoute(adaptive, force = false) {
  const match = location.hash.match(/^#\/lesson\/(VG0[1-3])$/);
  const main = document.querySelector('#main');
  if (!match || !main) return;
  const lesson = REPAIR_LESSONS.find(x => x.id === match[1]);
  if (!lesson) return;
  if (!force && main.dataset.runtimeLesson === lesson.id) return;
  const progress = adaptive.repairProgress[lesson.id] || { answers:{} };
  main.dataset.runtimeLesson = lesson.id;
  main.innerHTML = `<article class="lesson-shell extension-lesson"><div class="lesson-top"><button class="btn ghost small-btn" data-nav="improve">← Improve</button><div class="eyebrow" style="margin-top:20px">${label(lesson.skill)} · Repair</div><h1 class="lesson-title">${esc(lesson.title)}</h1><div class="meta"><span>${lesson.cefr}</span><span>${lesson.estimatedMinutes} min</span><span>Difficulty ${lesson.difficulty}/5</span>${progress.completed?'<span class="chip success">Completed</span>':''}</div><p class="lede">${esc(lesson.objective)}</p></div><section class="lesson-section"><h2>1. Learn</h2>${lesson.learn.map(x=>`<p>${esc(x)}</p>`).join('')}<div class="callout"><strong>Notice these patterns</strong><br>${lesson.examples.map(esc).join(' · ')}</div></section><section class="lesson-section"><h2>2. Guided Practice</h2>${lesson.questions.map((q,i)=>repairQuestion(lesson,q,i,progress.answers?.[i])).join('')}</section><section class="lesson-section"><h2>3. Review and reuse</h2><p>Explain the rule in your own words, then produce one new example.</p><textarea class="text-area" data-runtime-note="${lesson.id}" placeholder="My own example...">${esc(progress.note||'')}</textarea></section><section class="lesson-section"><div class="cluster" style="justify-content:space-between"><div><div class="eyebrow">Finish</div><h2 style="margin-top:6px">Repair → retry → review</h2></div><button class="btn primary" data-lrv="repair-complete" data-rid="${lesson.id}">${progress.completed?'Completed ✓':'Mark repair complete'}</button></div></section></article>`;
}

function apply(forceRepair = false) {
  if (applying) return;
  applying = true;
  try {
    ...
    renderRepairRoute(adaptive, forceRepair);
...
window.addEventListener('hashchange', () => setTimeout(() => apply(true), 0));
