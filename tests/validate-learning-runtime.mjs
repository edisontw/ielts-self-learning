...
const runtime = fs.readFileSync(new URL('../learning-runtime-v2.js', import.meta.url), 'utf8');
const assert = (condition, message) => { if (!condition) throw new Error(message); };

assert(index.includes('./learning-extension.css'), 'Learning extension CSS must be loaded.');
assert(index.includes('./learning-runtime-v2.js'), 'Stable learning runtime v2 must be loaded.');
assert(!index.includes('./learning-runtime.js"'), 'Broken/experimental runtime must not be loaded.');
...
assert(runtime.includes("#/lesson/${oldRepair.dataset.repairId}"), 'Repair cards must route to standard lesson URLs.');
...
console.log('✓ Stable learning runtime is mounted');
console.log('✓ VG01–VG03 use lesson routes');
console.log(`✓ Vocabulary Review: ${VOCABULARY_ITEMS.length} lesson-based items`);
console.log('✓ Skill performance feeds adaptive profile');
