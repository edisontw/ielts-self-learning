...
assert(Array.isArray(manifest.assets) && manifest.assets.length >= 22, 'Audio manifest must register ML01–ML04, all 12 QL depth assets, Placement and L01–L05.');
...
const expected = {
  ...
  "PLACEMENT": {
    path: "media/audio/placement-listening.mp3",
    duration: [75, 81],
    sha256: "2b101aa300a35ae9ffd94ff96f921393f619aa23c82cbe08df434a12ba10d336",
    sizeBytes: 1894025,
    statuses: ["production-ready", "production-live"]
  },
  "L01": {
    path: "media/audio/l01-listen-for-meaning.mp3",
    duration: [59, 63],
    sha256: "9d8c9e1041a2b31feab4a1cc97ecc631a7afe16f2c4c0f84d316145ff2998321",
    sizeBytes: 1470841,
    statuses: ["production-ready", "production-live"]
  },
  "L02": {
    path: "media/audio/l02-connected-speech.mp3",
    duration: [32, 35],
    sha256: "38d28e7e633d623c57fd8dc9661d64397de67ffa34b5e01386f621a331a1dbc5",
    sizeBytes: 772431,
    statuses: ["production-ready", "production-live"]
  },
  "L03": {
    path: "media/audio/l03-listening-paraphrase.mp3",
    duration: [29, 32],
    sha256: "20bb0d7284b30ec76f54da2dc77ce441b2d02ba022be04ad2e44b6fc9c9ab500",
    sizeBytes: 716007,
    statuses: ["production-ready", "production-live"]
  },
  "L04": {
    path: "media/audio/l04-distractors.mp3",
    duration: [44, 49],
    sha256: "1d23f700d497e611799f77c0bf1c7824aff5ef377b1c99ed092fc993230ff9c7",
    sizeBytes: 1091543,
    statuses: ["production-ready", "production-live"]
  },
  "L05": {
    path: "media/audio/l05-predict.mp3",
    duration: [26, 30],
    sha256: "b99ee5751771ea383b865e7079cff5a019d22841a64d520c3c9262abb8d6b1aa",
    sizeBytes: 663971,
    statuses: ["production-ready", "production-live"]
  }
};
...
assert(manifest.assets.find(item => item.id === 'PLACEMENT')?.qa?.tempoCorrection?.includes('143 wpm'), 'Placement must retain its final pacing correction QA.');
assert(manifest.assets.find(item => item.id === 'L01')?.qa?.assembly?.includes('Maya and Daniel'), 'L01 must retain its two-speaker assembly QA.');
assert(manifest.assets.find(item => item.id === 'L04')?.qa?.assembly?.includes('Clerk and Traveller'), 'L04 must retain its distractor-dialogue assembly QA.');
assert(manifest.assets.find(item => item.id === 'L05')?.qa?.contentApproval?.includes('Patel'), 'L05 must retain surname-pronunciation content QA.');
...
console.log('✓ ML01–ML04, all 12 QL recordings, Placement and L01–L05 exist at their runtime paths');
console.log('✓ Audio file sizes and SHA-256 checksums match the QA-approved assets');
console.log('✓ Duration, pacing, pause-processing and dialogue-assembly contracts are enforced');
