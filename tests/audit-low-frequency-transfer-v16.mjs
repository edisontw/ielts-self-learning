...
assert(academicVocabulary.filter(x=>x.skill==='listening').length===1&&academicVocabulary.filter(x=>x.skill==='reading').length===1,'Academic-vocabulary must remain one signal per skill, not a two-signal same-skill family.');
...
assert(mockDefinition[0].tag==='definition','Single skill-aware Full Mock definition evidence does not need a cosmetic listening- prefix.');
...
assert(mockSpelling[0].tag==='spelling','Single skill-aware Full Mock spelling evidence does not need a cosmetic listening- prefix.');
assert(noOwnership('spelling')&&noOwnership('listening-spelling'),'Single Full Mock spelling signal must not create runtime route/Repair ownership before recurrence is established.');
...
console.log('✓ KEEP-SKILL-AWARE: MA01-L01 spelling is already semantically precise inside Listening; QL02 is the exact owner, but one Full Mock signal does not justify a route/Repair or cosmetic retag.');
console.log('✓ KEEP-SKILL-AWARE: MA01-L31 definition is one explicit Listening definition-retrieval signal; no cosmetic retag or intervention is justified from one item.');
console.log('✓ KEEP-SPLIT-BY-SKILL: academic-vocabulary = one Listening + one Reading Full Mock item; aggregate count 2 is not same-skill recurrence.');
console.log('✓ Discovery gate: all remaining one-off/two-off Test/Mock signals are semantically accounted for; none creates a new Repair lesson or existing-practice route.');
