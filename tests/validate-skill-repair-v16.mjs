...
assert(rr01 && lr01 && V16_SKILL_REPAIR_LESSONS.length === 2, 'V1.6 Batch 1 must register exactly RR01 and LR01');
...
assert(rr01.evidence?.auditedQuestions === 23 && lr01.evidence?.auditedQuestions === 27, 'Lesson evidence metadata must preserve audited counts');
...
assert(routeRuntime.includes('question.context'), 'Unified Repair route must render RR01 reading contexts');
...
console.log('✓ Bare Full Mock tags cannot cross-route between Reading and Listening Skill Repair');
console.log('✓ RR01 / LR01 remain outside Vocabulary / Grammar Repair and the 30-unit core denominator');
