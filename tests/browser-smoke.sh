...
smoke_existing_practice_routing() {
  local dom="/tmp/ielts-dom-v16-existing-practice.html"
  local log="/tmp/ielts-chrome-v16-existing-practice.log"
  local profile="/tmp/ielts-chrome-profile-v16-existing-practice"
  rm -rf "$profile" "$dom" "$log"
  set +e
  timeout 45s "$CHROME" \
    --headless=new \
    ...
    --window-size="1280,900" \
    --virtual-time-budget=14000 \
    --dump-dom "$BASE/tests/browser-existing-practice-routing-v16.html" >"$dom" 2>"$log"
  local status=$?
  set -e
  if [[ "$status" -ne 0 && "$status" -ne 124 ]]; then
    echo "V1.6 existing-practice Chrome exited with status $status" >&2
  fi
  if ! grep -Fq 'V16_EXISTING_PRACTICE_PASS' "$dom"; then
    echo "Browser interaction failed for V1.6 existing-practice routing." >&2
    cat "$dom" >&2 || true
    cat "$log" >&2 || true
    exit 1
  fi
  echo "Browser interaction passed: skill-aware existing-practice routes, Error Notebook CTAs, original Retry priority, and R02 navigation"
}
...
echo "Browser smoke passed across core desktop routes, V/G Repair, V1.6 RR01/RR02/LR01 Skill Repair, existing Core/Lab error routing, Learn Repair indexes, V1.3 Lab depth routes, eight Mini Tests, Academic Writing Task 1, Speaking Practice Bank and mobile IELTS navigation."
